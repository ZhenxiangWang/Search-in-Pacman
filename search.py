...
def uniformCostSearch(problem):
    ...
    statePriorityQueue.update((problem.getStartState(), [], 0), 0)
    while not statePriorityQueue.isEmpty():
        currentState, action, cost = statePriorityQueue.pop()
        if problem.isGoalState(currentState):
            return action
        if currentState in visitedState:
            continue
        visitedState.add(currentState)
        for state, nextAction, nextCost in problem.getSuccessors(currentState):
            if state not in visitedState:
                statePriorityQueue.update((state, action + [nextAction], cost + nextCost), cost + nextCost)
    return None
    #util.raiseNotDefined()
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not statePriorityQueue.isEmpty():
        ...
        for state, nextAction, nextCost in problem.getSuccessors(currentState):
            if state not in visitedState:
                statePriorityQueue.update((state, action + [nextAction], cost + nextCost),
                                          cost + nextCost+ heuristic(state,problem))
    return None
